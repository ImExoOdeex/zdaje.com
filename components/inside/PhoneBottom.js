...
function PhoneBottom({ average, gradesLenght }) {
    ...
    const averageColor = average > 1.74 ? 'green.500' : 'red.500';
    ...
    return (
        <Flex as={motion.div} initial={initial} animate={visible} pos={'fixed'} h='70px' alignItems={'center'} justifyContent='center' display={{ base: 'flex', sm: 'none' }}
            ...
            <Heading fontSize={'40px !important'} color={average == "???" ? tealColor : averageColor}>{average}</Heading>
        </Flex>
    )
}
