...
export function HelpCard({ children, ...props }) {
    ...
    return (
        <WrapItem as={motion.div} layout w='100%' display={'block'} boxShadow={'md'} bg={bg} p={5} rounded='md' justifyContent={'space-around'} alignItems='center' {...props}>
            ...
            <Text as={motion.p} layout m={[0.5, 1, 2]}>Jeśli twoja ocena cząstkowa posiada minusa to odejmij od niej 0.25, a jeśli posiada plusa to dodaj
                do niej 0.5. Na przykład ocena 3+ to 3.50, a 3- to 2.75. W twojej szkole może być to inaczej, ale w większości jest tak jak powyżej.</Text>
            {children}
        </WrapItem>
    )
}
